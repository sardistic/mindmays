...
- A separate classic edition with 100 connected fixed-view chambers, sixteen original period-rendered room plates (including four entirely unoccupied object chambers), painted-door navigation with no turning, and a compact 1990s encyclopedia-game interface
...
- Five chambers have had the window in their far wall boarded into a door, so a fork can be three ways; the painted doors otherwise open onto the chambers actually to your left and right, and the keep is built so those two alone are always enough: no chamber is ever entered facing two blank walls, all one hundred are reachable without turning, and two doors are offered on 42% of entries. The route board draws the chambers you have walked with every opened seal marked on the edge between them
...
- The seal waits for you to read it: the verdict and its explanation stay on screen until you press Continue, which reads "Walk through", "Try again" or "Leave the keep" depending on what happens next
- Your best run is kept when everything else is cleared, and the menu reports it with the rank it earned
